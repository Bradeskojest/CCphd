...
\begin{definition}[predicate $userOrdered$]\label{def:userOrdered}
...
\begin{definition}[predicate "$equals$"]\label{def:pred_equals}
...
\begin{definition}[predicate "$greaterThan$"]\label{def:greaterThan}
...
To be able to ask about the Wi-fi signal, we need a vocabolary for storing this
...
\begin{definition}[Predicate $hasGoodWifi$]\label{def:hasGoodWifi}
...
On top of just follow-up questions, we can define follow-up conversation which
...
\begin{equation}\label{rule:autoIs}
\begin{gathered}
 \forall u \forall x \exists y \forall z:
 is (u,User) \land \\
 is(x,y) \land \\
...
\begin{definition}[Predicate $userAgrees$]\label{def:userAgrees}
...
\begin{definition}[Predicate $userDoubts$]\label{def:userDoubts}
...
\begin{definition}[Predicate $userDontKnow$]\label{def:userDontKnow}
...
Now the \emph{Procedural Component} does the following steps:
\begin{enumerate}
...
%subsection
\subsection{Step 2. What kind of place this is?}
\label{section:step2}
...
\begin{gathered}
    Q:name(x,"restaurant")\land \\ % TERM?=x
    argIs(is,2,y)\land \\ %ARGIS?=y
    is(x,y)\land \\
    \left(
    \begin{tabular}{c}
...
%subsection
\subsection{Step 3. Does Joe's Pizza have Wi-Fi?}
\label{section:step3}
...
%subsection
\subsection{Step 5. What's on the menu at Joe's Pizza?}
\label{section:step5}
...
of the predicate $userOrdered$ via the query defined in the asseertion 
...
\emph{CC} KA Engine can know that this answer is actually inconsistant, because
the previous query (qeury \ref{q:orderedConstraing1}) would otherwise return
...
"I've never heard of food or drink called 'car' before. Are you sure it's a type of food or drink?". This question is again constructed from a textual template,
...
%subsection
\subsection{Step 8. What did you order? The second time}
\label{section:step8}
Since nothing changed in the KB that would cause the retraction of the 
$ccWantsToAsk(User1,userOrdered(User1,Visit1,x))$ question that appeared in the
...
responds with "I've never heard of 'Pizza Deluxe' before. Whhat kind of
...
the user to imediatelly place the new concept correctly, or reject it and 
...
\end{gathered}
\end{equation}
This way the system learnt about a completely new concept and placed it properly
into the existing KB hierarchy.

Additionally, after this is inserted, the new knowledge triggers the inferenece
...
for a diffeerent user $User2$. To see how it constructs, and answer this 
...
was provuided by other users. One assertion like this is 
...
kwnoeldge about the user (what he agrees with) and used by other inference
