...
Now we can define some more nlPatterns, to be able to cover fully the
...
yet confirmed, it causes the inference to produce a yes/no question:
"Are you at Z?".
...
done with a slightly more complicated approach (using custom predicate), whic
...
(\autoref{tab:conversation1}). Because the main question has novariables, it
can be only true or not, so \emph{CC} already provides the "yes/no" options
as part of the possible answer.

Once user answers, the assertion $userLocation(User1,JoesPizza)$ will get
asserted into the KB. This will also cause inference engine to retract these two
questions, since user location is not unknown anymore. At the same time the
\emph{Procedural Component} will assert that the place that user visited is
