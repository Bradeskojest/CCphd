...
the sentence $menuItem(Restaurant1,Coffee)$, checks into the KB, whether it 
...
Now, consider, our Logic to NL engine gets the task to convert the query 
$\\menuItem(Restaurant1,x)$, where $x$ represnets a variable as in other our 
...
\hl{Maybe add Logic to NL KB current state imeage}
%subsubsection
...
For illustration consider inverting the example defined in (17). The system is presented with the statement Joes Pizza has pizzas on the menu. Because all the textual representations of the concepts are indexed, it is able to find the concepts:
	Restaurant1
	menuItem
	Pizza
Because of the is and subclass knowledge of Restaurant1 and Pizza classes, which can be observed in Fig. 4, and the argIs and argClass domain limits of menuItem (definition 9), the inference engine is actually able to construct the logical equivalent of the above sentence: menuItem(Restaurant1, Pizza).
