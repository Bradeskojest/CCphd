...
Now lets define the rest of predicates and rules that are used in our examples 
from \autoref{tab:conversation1}. and KA dialog formulation.

\begin{definition}[Predicate $hasWifi$]\label{def:hasWifi}
To be able to ask about the Wi-fi signal, we need a vocabolary for storing this
information and also for asking. This is done by introducing $hasWifi$ 
predicate, formally defined as follows:
\begin{equation}\label{as:hasWifi}
\begin{gathered}
	is(hasWifi, Predicate) \land \\
	arity(hasWifi,1) \land \\
	argIs(hasWifi,1,PublicPlace) \land \\
	nlPattern(hasWifi,\text{"Does \$1 hafe Wi-Fi"})
\end{gathered}
\end{equation}
Then, the KA rule that triggers questions about Wifi:
\begin{equation}\label{rule:haswifi}
\begin{gathered}
	\forall x\forall u: userLocation(u,x) \land
	is (x,PublicPlace) \land unknown(hasWifi(x))\\
	\implies \\
	ccWantsToAsk(hasWifi(x))
\end{gathered}
\end{equation}
\end{definition}
...
%subsection
\subsection{What kind of place this is?}
\label{section:step2}
...
%subsection
\subsection{Does Joe's Pizza have Wi-Fi?}
\label{section:step3}
The assertion ($is(JoesPizza,Restaurant$)) which got asserted as a consequence
of answering step \ref{step:kind} from the \autoref{tab:conversation1} (see
