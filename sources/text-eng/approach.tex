...
\begin{gathered}
S_{Place} = \{Service,WirelessService,Vehicle,Car\}
\end{gathered}
\end{equation}
Where each of these terms is also an instance of a $Class$:
\begin{equation}\label{set:placeTermsClass}
...
Which are connected into the appropriate $subclass$ hierarchy:

\begin{equation}\label{as:kbSubclasses}
\begin{gathered}
	subclass(PublicPlace,Place) \land subclass(Restaurant,PublicPlace) \land \\
    subclass(Drink,FoodOrDrink) \land subclass(Coffee,Drink) \land \\
	subclass(Food,FoodOrDrink) \land subclass(Bread,Food) \land \\
	is(Baguete, Bread)
\end{gathered}
...
Then, each predicate needs a little bit more of logic, since we also want to
represent the constraints which will serve for the inference engine to check
