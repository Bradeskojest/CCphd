...
stating $is(Luka, Human)$ defines Luka as one instance of the class of humans.
\end{definition}
...
At this point we can define the rest of our predicates:
...
predicate (\hl{ref to def}). To make this definition more understandable, let's
...
Instead of writing full rule, this constraint for the argunent of $subclass$

\begin{definition}[predicate "$argIs$"]\label{def:pred_subclass}
Predicate sublcass(x,y) denotes that x is a subclass of y. For example, 
asserting $subclass(Dog, Animal)$, is spcifying all dogs, to be a sub-class of
animals.
\end{definition}
