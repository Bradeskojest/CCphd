...
\begin{gathered}
is(Something,Class) \land is(Class,Class) \land is(Predicate,Class)\\ 
...
asserting $subclass(Dog, Animal)$, is spcifying all dogs, to be a sub-class of
animals.
\end{definition}
...
example, to lf both $subclass$ arguments, to only allow
instances of a $Class$, we could assert:
...
Similary, we can now proceed to define our $is$ predicate constraints, which is
...
($Predicate$,$Class$), but
for the first argument we can actually put in everything ($is$, $Class$, 
$Something$, $Predicate$,  $Number$, $\mathbb{N}$).
\begin{equation}\label{as:is_domain}
\begin{gathered}
argIs(is,1,Something) \land argIs(is,2,Class) \land\\
argIs(subclass,1,Class) \land argIs(is,2,Class) \land argIs(arity,1,Predicate) \land\\
argIs(arity,2, Number) \land argClass(argIs,3,Something)
\end{gathered}
\end{equation}

\begin{equation}\label{as:alldomains}
\begin{gathered}
argIs(argClass,1,Predicate) \land argIs(argClass,2,Number) \land \\
argIs(argClass,3,Class) \land argIs(is,1,Something) \land argIs(is,2,Class) \land\\
argIs(subclass,1,Class) \land argIs(is,2,Class) \land argIs(arity,1,Predicate) \land\\
argIs(arity,2, Number) \land argClass(argIs,3,Something)
\end{gathered}
\end{equation}
