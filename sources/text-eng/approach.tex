...
Now we can also define the $name$ predicate, which is used to
add to concepts their name string representations.

\begin{definition}[predicate "$name$"]\label{def:pred_name}
...
And similarly, $nlDeclerative$ and $nlInterrogative$ predicates, attaching a
NL generation information to other predicates:

\begin{definition}[predicate "$nlDeclarative$"]\label{def:predNlDeclarative}
Predicate $nlDeclarative(x,y)$ denotes that the predicate $x$ has a nl pattern
that can be used in declerative sentences given as $y$. For example, asserting 
$nlDeclerative(is,\text{"\$1 is a \$2"})$, states that the predicate is, can
be represented in natural language as the name of first argument, followed by
the string "is a", and then followed by the name of the second argument. 
Formally the predicate is defined as:
\begin{equation}\label{as:predName}
\begin{gathered}
	is(name,Predicate)\land\\
