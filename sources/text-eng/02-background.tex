...
\begin{landscape}
	...
	\begin{tabular}{lclcccccc}
	    ...
	    PIP (NU) & PIP & \parencite{Sharma2010} & Reasoning & Induction &  CycL & \checkmark & / & / \\
		\hline
	\end{tabular}
	\end{adjustbox}
\end{table}
...
\subsection{Mindpixel}. \todo{write this: https://en.wikipedia.org/wiki/Mindpixel}
...
\subsection{KRAKEN} system was a knowledge entry tool which allows domain experts to
...
\subsection{Predicate Populator} is a similar tool as \emph{Factivore}, which instead
...
\subsection{Rapport} is a KA game based on Chinese OMCS questions, but implemented as a 
...
\subsection{Goal Oriented Knowledge Collection (GOKC)}. This game builds on the
...
especially its predicate hierarchy represented by \emph{genlPred} predicate, and
\emph{PredicateType} which represents second order collection of predicates that
can be grouped together by some common features.For example
\emph{(genlPreds holds touches)}, means that each time something is holding 
something else, it is also touching it. The system scans the KB for rules 
...
$((familyRelationsSlot(x,y)) \land (familyRelationSlot(y,z)) \implies 
