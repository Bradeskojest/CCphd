...
we address the problem by combining the labour and interaction acquisition, 
...
One of the most similar systems (by functionality and approach) is 
...
aspects, is \emph{Cyc CURE} KA system (\autoref{section:r:cure} whic is 
...
or crowdsourcing. It simply lists a set of quesions when \emph{Cyc} user clicks
on the concept inside Cyc's unternak web-gui.
...
On the other end of the spectrum lie conversational agents (chatbots) with a 
vast number of hand-scripted NL patterns and responsesi.
Recently, some successfull chatbots have started to employ an internal KB, in
addition to just scipted requests and response (subsections 
...
matches as with chatbots, \emph{Curious Cat} generates NL responses based on 
the knowledge and context driven inference. This allows our system to 
proactively engage users in sensible conversations and is thus one of the first 
approaches that attempts to bridge the gap between maintaining the NL 
conversation of chatbots and actually understanding its content.
...
knowledge (\emph{ConceptNet}) and automated quesion generation was implemented 
...
2006, the effort of making Cyc was 900 non-crowdsourced 
...
\subsubsection{Content Understanding, Review, or Entry(CURE)}
...
clickable and they cause the knowledge entry process for clicked concept.
...
of imediatelly switching the topic (see \autoref{tab:topics} in 
...
factor and un-transferrability of the knowledge. This is done by allowing
users to buld their own games (short, or long adventure games), which they can
share with friends who play. While the users are designing a game, they
also generate knowledge as a side effect when defining screens, iteractions
between the objects and other users. Authors call these knwoledge pieces
...
via natural langauge dialogue.  Over the course of 8 months, 70,000
...
4290 searches. It found 1016 facts, out of which 4 were rejectec
